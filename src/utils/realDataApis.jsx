...
export const fetchAllNewsFeed = async () => {
  try {
    const [techNews, sportsNews, cryptoNews] = await Promise.all([
      fetchRealTechNews(),
      fetchRealSportsNews(),
      fetchRealCryptoNews()
    ]);
    ...
    const allPosts = [...techNews, ...sportsNews, ...cryptoNews]
      .sort(() => Math.random() - 0.5)
      .slice(0, 50); // Limit to 50 posts max

    return allPosts;
  } catch (error) {
    console.error('Error fetching all news feed:', error);
    
    // Fallback to individual API calls if combined fails
    const techNews = await fetchRealTechNews();
    const sportsNews = await fetchRealSportsNews();
    const cryptoNews = await fetchRealCryptoNews();
    
    return [...techNews, ...sportsNews, ...cryptoNews]
      .sort(() => Math.random() - 0.5)
      .slice(0, 50);
  }
};
